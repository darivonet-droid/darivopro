...
export function NuevoCotizacionWizardEscritorio() {
  ...
  // 3 pasos oficiales — 05-MODULO-COTIZACIONES-EMPRESA.md §4/§5
            ...
            return (
              <div
                ...
                style={{
                  ...
                  background: sel ? catActiva.color + "0A" : ADMIN_COLORS.white,
              ...
              >
                ...
                {p.calcType !== "fixed" && (
                  <input
                    ...
                    inputMode="decimal"
                    value={it?.qty ?? ""}
                    onChange={(e) => setCantidad(catActiva, p, e.target.value)}
                    placeholder="0"
                    style={{ width: 56, padding: "8px 6px", borderRadius: 10, fontSize: 15, fontWeight: 800, border: `1.5px solid ${sel ? catActiva.color : ADMIN_COLORS.slateD}`, outline: "none", color: ADMIN_COLORS.text, background: ADMIN_COLORS.slate, textAlign: "center", fontFamily: "inherit", flexShrink: 0 }}
                  />
                )}
                <button
                  type="button"
                  onClick={() => toggleSvc(catActiva, p)}
                  style={{ width: 30, height: 30, borderRadius: 9, background: sel ? catActiva.color : ADMIN_COLORS.slate, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, border: "none", cursor: "pointer" }}
                >
        ...
        <div>
          ...
          <div style={{ background: ADMIN_COLORS.white, borderRadius: 14, border: `1px solid ${ADMIN_COLORS.slateD}`, padding: "14px 16px", marginBottom: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
              ...
              <span style={{ fontSize: 15, fontWeight: 900, color: ADMIN_COLORS.purple }}>{margin % 1 === 0 ? margin : margin.toFixed(1)}% · {fmtPEN(totalLabor)}</span>
            </div>
            <input type="range" min={0} max={120} step={5} value={margin} onChange={(e) => { setMargin(parseInt(e.target.value)); setMontoManoObra(""); }} style={{ accentColor: ADMIN_COLORS.purple, width: "100%", marginBottom: 8 }} />
            <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
              {[0, 25, 40, 60, 80, 100].map((v) => (
                <button key={v} type="button" onClick={() => { setMargin(v); setMontoManoObra(""); }} style={{ flex: 1, padding: "7px 2px", borderRadius: 10, border: `1.5px solid ${margin === v ? ADMIN_COLORS.purple : ADMIN_COLORS.slateD}`, background: margin === v ? ADMIN_COLORS.purplePale : "none", cursor: "pointer", fontSize: 11, fontWeight: 800, color: margin === v ? ADMIN_COLORS.purple : ADMIN_COLORS.textMid }}>
                  {v}%
                </button>
              ))}
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ fontSize: 12, fontWeight: 700, color: ADMIN_COLORS.textMid }}>S/</span>
              <input
                type="text"
                inputMode="decimal"
                value={montoManoObra}
        ...
        <div style={{ position: "sticky", bottom: 16, display: "flex", alignItems: "center", justifyContent: "space-between", background: ADMIN_COLORS.purpleDark, borderRadius: 16, padding: "14px 20px", boxShadow: "0 8px 24px rgba(10,22,40,0.25)" }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button
              ...
              style={{ padding: "10px 20px", borderRadius: 12, border: "none", background: ADMIN_COLORS.purple, color: ADMIN_COLORS.white, fontSize: 13, fontWeight: 800, cursor: cantidadesCompleto ? "pointer" : "not-allowed", opacity: cantidadesCompleto ? 1 : 0.5 }}
            >
              Continuar → Resumen
            </button>
          </div>
