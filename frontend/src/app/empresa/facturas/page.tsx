...
export default async function EmpresaFacturasPage() {
  ...
  const [facturasRes, perfilRes, aprobadosRes] = await Promise.all([
    ...
    supabase
      .from("presupuestos")
      .select(
        "id, user_id, client_name, phone, city, margin, total_base, total_labor, total_final, status, created_at, notes"
      )
      .eq("status", "Aprobado")
  ...
  const mod = empresaModulo("facturas");
