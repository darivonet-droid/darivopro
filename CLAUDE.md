...
**Roles personalizados (Empresa §6.1) — APROBADOS el 23/07/2026, pendientes de construcción real (FASE 1 hecha, solo documentación).** El propietario aprobó la propuesta tal como estaba escrita: solo plan Business, nombre único y no reservado, mismo catálogo de permisos de la matriz cerrada, asignable a varios Técnicos como plantilla editable, y al eliminar el rol sus Técnicos vuelven a Técnico base. Documentado en `11-ROLES-PLANES-PERMISOS-EMPRESA.md` v1.7 §6.1, `12 – ROLES… ADMIN.md` v1.7 §7.2 y `01-VISION-DEL-PRODUCTO.md` v2.21 §8. Pendiente aparte y **solo visual, no bloquea backend**: la nueva imagen oficial.

⚠️ **Hallazgo importante del audit: "pendiente de construcción" no era exacto.** Buena parte ya existe desde el 06/07/2026 — tabla `roles_personalizados` con RLS, `empresa_empleados.rol_personalizado_id`, límite por cuenta en `suscripciones.limite_roles_personalizados`, y pantalla completa de crear/editar/eliminar/asignar en `RolesPermisosView.tsx`. **Lo que falta de verdad (FASE 2)** es: (1) el **enforcement** — los permisos del rol se guardan pero no gatean ninguna ruta ni acción (`matriz-permisos.ts` lo declara "RBAC custom inerte"); (2) alinear `MODULOS_PERMISO_ROL` (7 módulos en código) con la matriz aprobada (Factura/Informe) — además el toggle de **Mis Tarifas** contradice la regla "el Técnico no administra Mis Tarifas"; (3) el badge "🔒 Solo lectura" de una pantalla que sí es interactiva. **Nada de esto se tocó** — es Fase 2.
