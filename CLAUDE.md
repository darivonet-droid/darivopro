...
**EN PRODUCCIÓN (23/07/2026)** — PR #10 `develop`→`main` mergeado (merge `7b84c92`), deploy Vercel `dpl_khfnK1J8Jpbc8MrsF5FpgJGC4d63` en `READY`/`production`, aliaseado a `darivopro.com` + `admin.` + `empresa.` + `partners.`. Smoke test: los 3 dominios responden HTTP 200. `build`/`lint`/`typecheck` en verde antes del merge (los 3 warnings de `react-hooks/exhaustive-deps` en `useCotizacion.ts`/`useFactura.ts` son preexistentes y ajenos). Payload: los 8 commits del 23/07 (25 archivos) — el resto de `develop` ya estaba en `main` vía PR #7/#8/#9.

**Las 2 migraciones del 23/07 ya están aplicadas en la base de producción** (`vyrtokggypcmpforglch`, la misma que sirve `darivopro.com` — no hay staging). Verificado por API REST con `service_role`: `admin_plan_auditoria` responde HTTP 200, y `cotizaciones`/`cotizacion_items` responden **HTTP 403 `42501 permission denied`**, que es exactamente el efecto buscado del REVOKE.

⚠️ **Nota de secuencia, para no repetir el error:** el REVOKE (`20260723120000`) se aplicó **antes** del deploy del código nuevo, cuando `main` todavía leía `cotizaciones` con `service_role`. No provocó caída —`fetchAdminDashboard()` desestructuraba solo `data` e ignoraba `error`, así que el 403 se degradaba a `data: null` → lista vacía y serie en 0—, pero el Dashboard de Admin estuvo mostrando datos incompletos en silencio hasta este deploy. El orden correcto en un cambio así es **desplegar el código primero y revocar después**. Este deploy cierra la degradación: el código nuevo ya no consulta esa tabla.
...
- **Gestión de planes: Usuarios → Suscripciones** (decisión del propietario, 23/07/2026 — el plan de una cuenta es metadato de facturación; el operador lo conserva, pero en Suscripciones). Orden **aditivo obligatorio**: construir y verificar antes de quitar nada.
  ...
  - **FASE B — desbloqueada, pendiente de prueba funcional.** La tabla `admin_plan_auditoria` ya existe en producción (verificado) y el código está desplegado. Falta la prueba en vivo: cambiar el plan de una cuenta desde Admin → Suscripciones → Cuentas y confirmar que persiste y aparece en "Historial de cambios".
  - **FASE C — NO EJECUTADA a propósito.** Quitar "Cambiar plan" de Admin → Usuarios está condicionado a que FASE B esté verificada en vivo; hasta entonces Usuarios conserva su punto de entrada (ya auditado) para no dejar el producto sin ninguna vía.
...
**Roles personalizados (Empresa §6.1) — FASE 1 (aprobación) + FASE 2 (construcción) CERRADAS el 23/07/2026.** El propietario aprobó la propuesta (solo Business, nombre único no reservado, catálogo cerrado, asignable a varios Técnicos, al eliminar vuelven a Técnico base) y luego autorizó construir Fase 2 con reglas explícitas. Documentado en `11-ROLES-PLANES-PERMISOS-EMPRESA.md` v1.8 §5.2/§6.1/§6.2, `12 – ROLES… ADMIN.md` v1.7 §7.2 y `01-VISION-DEL-PRODUCTO.md` v2.21 §8. Pendiente aparte y **solo visual, no bloquea backend**: la nueva imagen oficial.

✅ **FASE 2 construida (B + A), typecheck/lint/build en verde.**
- **B1** — `MODULOS_PERMISO_ROL` recortado a **Factura + Informe** (catálogo cerrado §5.2); el toggle de **Mis Tarifas** (contradecía "el Técnico no administra Mis Tarifas") eliminado de raíz del formulario.
- **B2** — el chip "🔒 Solo lectura" de `RolesPermisosView.tsx` (contradecía la capacidad real) reemplazado por un chip de estado real según el plan.
- **A (enforcement)** — `obtenerContextoAcceso()` (`lib/rol-empleado.ts`, punto único de gating del Técnico, consumido por las 5 rutas de `(auth)`) lee `roles_personalizados.permisos` y aplica la **regla de combinación** aprobada: **rol asignado ⇒ el permiso del rol REEMPLAZA los flags individuales** Factura/Informe; sin rol ⇒ rigen los flags (no se borran, quedan en pausa). Aislamiento confirmado en 3 capas (FK `empresa_id` + RLS `roles_personalizados_gerente` + guard `.eq(empresa_id)` en la lectura service_role) — **sin tocar RLS, sin migración** (solo lee columnas existentes). Sin fail-open (Paso 0 lo confirmó; claves ausentes ⇒ denegar).
- **EN PRODUCCIÓN (23/07/2026)** — PR [#11](https://github.com/darivonet-droid/darivopro/pull/11) `develop`→`main` mergeado (merge `91c86e6`), deploy Vercel `dpl_B8E64LtBVaVgQgCEzQg8DWrf1SPf` en `READY`/`production`, aliaseado a los 4 dominios (`aliasError: null`). Smoke test: `darivopro.com` + `admin.` + `empresa.` + `partners.` → **HTTP 200**. CI en verde (Test Frontend, Test Backend Python; Railway skipping). Payload: 6 commits (los 5 de roles + `121f394` doc). **Sin migración** (solo lee columnas ya existentes en prod).
- **Verificación en vivo — lado Gerente ✅, lado Técnico ⏳.** Con la cuenta QA `demo@darivopro.com` (Gerente Business) en `empresa.darivopro.com/empresa/roles` se confirmó en producción: **B2** el chip dice "✏️ Administras los roles y permisos de tus Técnicos" (ya no "Solo lectura"); **B1** el catálogo solo lista **Factura** e **Informe** (sin Mis Tarifas ni los otros 5 módulos); badge **Plan BUSINESS**. **Pendiente el lado Técnico (pasos 8c/9/10):** requiere iniciar sesión **como el Técnico** para observar que pierde Factura/gana Informe al asignarle el rol — no realizable en esta sesión (regla: nunca escribir contraseñas; solo hay una sesión de Google activa = el Gerente demo, no el Técnico). Observación neutra a revisar por el propietario en su prueba: la lista de roles quedó en "Cargando roles…" y la pestaña dejó de responder a la automatización (posible inestabilidad de la extensión, no confirmado como bug de app). Detalle: `docs-internos/tareas/2026-07-23-fase2-roles-personalizados-enforcement.md`.

**Audit de sincronización Admin↔Empresa↔Partner (23/07/2026):** 4 contradicciones **de texto** corregidas — (1) Visión decía que los roles personalizados se administran "desde Empresa y Móvil", cuando Móvil no administra roles (Admin §16); (2) Visión §8/§19 decían que el límite lo define el plan, cuando se configura **por cuenta** desde Suscripciones (columna real en BD); (3) referencia circular entre Suscripciones y el doc 12 sobre quién define ese límite; (4) versiones desactualizadas citadas en `04-ROLES-PLANES-PERMISOS-DARIVO-PRO.md`. **Sin contradicción**: nombres de rol, quién administra qué, y el Panel Partner (su MD dice expresamente que no documenta permisos, así que no puede contradecir). Detalle: `docs-internos/tareas/2026-07-23-fase1-roles-personalizados-aprobacion-audit-sincronizacion.md`.
...
- `20260723120000_revocar_cotizaciones_service_role.sql` — ✅ **ya aplicada** (23/07/2026, verificada: `cotizaciones`/`cotizacion_items` devuelven 403 a `service_role`).
- `20260723130000_admin_plan_auditoria.sql` — ✅ **ya aplicada** (23/07/2026, verificada: la tabla responde 200). Idempotente (`IF NOT EXISTS`), se puede volver a pegar sin error.
...
- [2026-07-23-fase2-roles-personalizados-enforcement.md](docs-internos/tareas/2026-07-23-fase2-roles-personalizados-enforcement.md) — construida y verificada en verde (pendiente verificación en vivo, sin mergear a main). Fase 2 B+A: catálogo del rol cerrado a Factura/Informe (toggle de Mis Tarifas eliminado), chip "Solo lectura" corregido, y enforcement real en `obtenerContextoAcceso()` con regla de combinación (rol reemplaza flag individual del Técnico). Aislamiento por empresa_id confirmado en 3 capas; sin tocar RLS ni migración.
- [2026-07-23-fase1-roles-personalizados-aprobacion-audit-sincronizacion.md](docs-internos/tareas/2026-07-23-fase1-roles-personalizados-aprobacion-audit-sincronizacion.md) — cerrada (solo documentación). §6.1 Roles personalizados aprobada por el propietario y sincronizada en Empresa v1.7 / Admin v1.7 / Visión v2.21; audit de sincronización con 4 contradicciones de texto corregidas y 4 hallazgos de comportamiento en código listados para Fase 2 (RBAC inerte, catálogo de permisos divergente, toggle de Mis Tarifas, badge "Solo lectura"). Sin tocar código.
