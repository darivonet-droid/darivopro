...
**⚠️ Hallazgo de proceso (13/07/2026):** el Service Worker de la PWA cachea agresivamente el HTML/JS de `/admin/*` — dos verificaciones visuales seguidas mostraron la versión ANTERIOR del código ya desplegado, hasta hacer `caches.keys()...delete()` + `serviceWorker.getRegistrations()...unregister()` vía consola antes de recargar. **A partir de ahora, toda verificación visual en producción debe limpiar el Service Worker/caché primero** (o abrir en pestaña de incógnito) — si no, se puede dar por buena una pantalla que en realidad sigue sirviendo el build viejo.
...
- El proyecto Supabase real de producción es **`kyckjapprmtfahnkuucz`** — confirmado extrayendo el bundle JS servido en vivo (`NEXT_PUBLIC_SUPABASE_URL`, público del lado cliente), usado consistentemente en toda la app.
...
- Otras tablas Admin (`perfiles`, `darivo_admin_empleados`, `partner_comisiones_config` vía REST directo) sí respondieron bien en pruebas — pero **no se ha confirmado con certeza que `empresas` funcione para NINGUNA pantalla en producción real**, dado este hallazgo. Dashboard/Empleados/Configuración muestran "0" en todo, consistente tanto con "no hay datos todavía" como con "esa tabla específica también falla" — no se puede distinguir sin acceso directo a `kyckjapprmtfahnkuucz`.

**No se tocó ninguna migración ni se ejecutó SQL alguno contra producción** — solo lectura (vía REST/`supabase-js` con la key local, que apunta a otro proyecto) y diagnóstico de logs de Vercel. **Pendiente de decisión del propietario — requiere acceso directo al dashboard de `kyckjapprmtfahnkuucz` que esta sesión no tiene:**

1. Confirmar cuál de los dos proyectos (`kyckjapprmtfahnkuucz` o `vyrtokggypcmpforglch`) es el que se debe considerar producción real de aquí en adelante.
2. Si es `kyckjapprmtfahnkuucz`: verificar en su SQL Editor si las tablas `partners`, `partner_comisiones_config`, `partner_comisiones_historial`, `partner_referidos` y `empresas` existen y tienen los grants/RLS esperados — el mensaje "Invalid API key" en el gateway de Supabase típicamente indica un problema a nivel de proyecto/gateway, no de código, pero solo se puede confirmar con acceso directo.
3. Efecto colateral bueno de esta sesión, ya aplicado: `adminClientOrNull()` (`admin-queries.ts`) ya no traga errores en silencio — cualquier fallo real de Supabase en cualquier pantalla Admin ahora queda en los logs de Vercel con detalle, en vez de mostrar el mensaje genérico "no configurada" indistinguible de un entorno mal configurado.
