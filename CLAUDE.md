...
- Empresa — **doc↔código desincronizado desde 22/07/2026:** la documentación (`03-darivo-pro-empresa/16-SISTEMA-DE-DISEÑO-EMPRESA.md` v2.7, `07-MODULO-MAS-EMPRESA.md` v2.0) ya no tiene un módulo "Más" en el sidebar — sus 7 funcionalidades pasan a entrada directa (posiciones 6, 9-14), por pedido explícito del propietario (ver `docs-internos/tareas/2026-07-22-eliminacion-modulo-mas-navegacion-directa.md`). El código de producción **todavía tiene el sidebar viejo con "Más"** — este cambio fue solo de documentación (fase de diseño/implementación pendiente, no iniciada). No dar por hecho que el sidebar real ya coincide con el MD hasta que se implemente y se verifique.
- Empresa (estado previo a la reorganización de navegación) — 9/9 pantallas auditadas MD↔código y verificadas visualmente, 9/9 migradas a `ADMIN_COLORS` (shell + contenido), **todo en producción**: los 5 componentes compartidos con Móvil (`ClienteFichaView.tsx`, `MasTabs.tsx`, `MasOpcionesList.tsx`, `IACotizacionFlow.tsx`, `InformesTab.tsx`) vía PR #4 (commit `274df88`, deploy `dpl_BnxJu5CgxdT5HxM1ozNUM8xmao3T`), verificados en vivo con la cuenta demo Business; y sus 3 hijos de Informes (`InformeSemanal.tsx`, `InformeMensual.tsx`, `InformeTrimestral.tsx`, hallazgo adicional) vía PR #5 (commit `04f20a3`, deploy `dpl_AZRuyii3bNdtHGaGpURW4HLfZg1B`, ambos `READY`/`production`) — **verificación visual en vivo de estos 3 pendiente** con la cuenta `yatriye@gmail.com` (Mohamed la está abriendo). Todos usan el patrón `esEmpresa` (prop booleana, gateo condicional del color) — Móvil sigue con el azul de Fable 5 sin ningún cambio, confirmado por código en cada punto de entrada. Pendiente, decisión aparte de Mohamed, sin tocar: `components/ui/Button.tsx` (variant `primary`, gradiente azul global compartido por Móvil). Detalle: `docs-internos/tareas/2026-07-22-verificacion-admin-colors-empresa.md`.
