...
- Empresa — 9/9 pantallas auditadas MD↔código y verificadas visualmente, 9/9 migradas a `ADMIN_COLORS` (shell + contenido), **en producción** (merge PR #4 a `main`, commit `274df88`, deploy Vercel `dpl_BnxJu5CgxdT5HxM1ozNUM8xmao3T` confirmado `READY`/`production`, verificado en vivo en `darivopro.com` el 22/07/2026 con la cuenta demo Business). Los 5 componentes compartidos con Móvil que quedaban en azul (`ClienteFichaView.tsx`, `MasTabs.tsx`, `MasOpcionesList.tsx`, `IACotizacionFlow.tsx`, `InformesTab.tsx`) y sus 3 hijos de Informes (`InformeSemanal.tsx`, `InformeMensual.tsx`, `InformeTrimestral.tsx`, hallazgo adicional) usan el patrón `esEmpresa` (prop booleana, gateo condicional del color) — Móvil sigue con el azul de Fable 5 sin ningún cambio, confirmado por código y en vivo (valores computados exactos: `T.bluePale`/`T.blue`, no `ADMIN_COLORS`). Pendiente, decisión aparte de Mohamed, sin tocar: `components/ui/Button.tsx` (variant `primary`, gradiente azul global compartido por Móvil). Detalle: `docs-internos/tareas/2026-07-22-verificacion-admin-colors-empresa.md`.
...
- [2026-07-22-verificacion-admin-colors-empresa.md](docs-internos/tareas/2026-07-22-verificacion-admin-colors-empresa.md) — cerrada. Corrigió "Empresa 9/9 migradas a ADMIN_COLORS" (eran 5/9), migró los 5 componentes compartidos con Móvil que faltaban + sus 3 hijos de Informes (patrón `esEmpresa`, Móvil sin cambios verificado en código y en vivo), llevó los 6 commits a `main` (PR #4, "producción autónomo") y confirmó el deploy y la verificación visual en `darivopro.com`. Ahora sí 9/9, en producción.
