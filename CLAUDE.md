...
- Empresa — sidebar sin "Más" implementado en código (`develop`, commit `eb8985f`, 22/07/2026): 14 entradas directas (Inicio, Clientes, Darivo/IA, Facturas, Cierre, Catálogo·Mis Tarifas, Empleados, Roles y Permisos, Empresa, Informes, Documentos, Mi Plan, Soporte, Configuración), sin agrupador "Más". `build`/`lint`/`typecheck` en verde. **Pendiente verificación visual en Vercel** (PR #7 `develop`→`main`, no mergeado — esperando aviso del propietario) y las 7 imágenes oficiales nuevas. Detalle: `docs-internos/tareas/2026-07-22-implementacion-navegacion-directa-empresa.md`.
