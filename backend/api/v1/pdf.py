...
from services.pdf.generator import generar_pdf_presupuesto, generar_pdf_factura
...
@router.post("/presupuesto", response_model=ApiResponse[dict])
async def pdf_presupuesto(
    presupuesto: dict,
    _: None = Depends(verify_service_key),
):
    try:
        url = await generar_pdf_presupuesto(presupuesto)
